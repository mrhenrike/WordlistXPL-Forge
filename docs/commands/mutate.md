...
usage: wlf.py mutate [-h] [--leet-mode {basic,v2,v3,all,none}]
                     ...
                     [--min-len MIN_LEN] [--max-len MAX_LEN] [--titlecase]
                     [-o OUTPUT]
                     password
...
options:
  ...
  --titlecase           Also seed a per-token CamelCase variant of the base
                        (e.g. nerd_trilha -> Nerd_Trilha)
...
| `--titlecase` | False | no | Also seed a per-token CamelCase variant of the base (e.g. `nerd_trilha` -> `Nerd_Trilha`) |
